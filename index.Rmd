...
files <- reactive({list.files(path = paste0(base.dir, input$forecast), pattern = input$forecast)})

file.date <- reactive({strapplyc(files(), pattern = "[0-9-]{8,}", simplify = TRUE) %>% 
            gsub("-$", "", .) %>% 
            gsub("^-", "", . )})
...
files.date = reactive(list.files(paste0(base.dir, input$forecast), pattern = input$date))
file.model =  reactive({files.date() %>% 
                str_extract(., "\\-[[:alpha:]](.*?)\\.") %>% 
                gsub("^-", "", .) %>% gsub(".$","",.)
})
...
data <- reactive({
  ...
  if(ext() == "gz"){  #if file is csv zip file
