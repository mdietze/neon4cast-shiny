...
target_variables <- c("oxygen", "temperature", "richness", "abundance", "nee", "le", "vswc", "gcc_90", "Ixodes_scapularis", "Ambloyomma_americanum")
#reads selected data 
data <- reactive({
  
  if(ext() %in% c("gz","csv")){  #if file is csv zip file
    print(file.path())
    d <- as.data.frame(read_csv(file.path(), guess_max = 100000))
    if("plotID" %in% names(d)){
      d %>% mutate(siteID = plotID)
    }else{
      d
    }
...
get_scores <- function(files){
  ...
  for(i in 1:length(teams_tmp)){
    ...
    if(nrow(curr_score) > 0){
      if(str_detect(teams_tmp[[i]][2], "30min")){
      ...
      if(teams_tmp[[i]][2] != "ticks"){
      combined <- tibble(date = dates,
