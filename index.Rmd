...
dirs <- reactive({
  files <- list.files(path = "/efi_neon_challenge/forecasts", recursive = FALSE)
...
file.date <- reactive({
  d <- lubridate::as_date(strapplyc(files(), pattern = "[0-9-]{8,}", simplify = TRUE) %>% 
                                            gsub("-$", "", .) %>% 
                                            gsub("^-", "", . ))
...
file <- reactive({
  print(files.date())
  files.date()[grep(input$model, files.date())]
  })


#creates files path from above selection
file.path <- reactive({
  paste0(base.dir, input$forecast, "/", file())
  })
...
x.inputs <- reactive({
  vars()[which(vars() %in% x_axis_variables)]
  })
...
index.y <- reactive({
  which(names(sub.data()) == input$y.var)
  })
...
data.sum <- reactive({
  ...
  if(ensemble()){
    ...
    d1 <- d %>% 
    mutate(var.y = var.y())  %>%  
      select(var.y, "statistic", "time", siteID) %>%  
